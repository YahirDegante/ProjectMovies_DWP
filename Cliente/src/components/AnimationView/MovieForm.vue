<template>
  <div class="mb-4">
    <b-card header="Arrastra el formulario para registrar" header-bg-variant="primary" header-text-variant="white"
      draggable @dragstart="startDrag($event, JSON.stringify(pelicula))">
      <b-form v-if="show">
        ...
        <b-form-group class="mb-3" id="input-group-1" label="Nombre de la pelicula:" label-for="input-1">
          <b-form-input id="input-1" v-model="pelicula.nombre" placeholder="Nombre de la pelicula" />
        </b-form-group>

        <b-form-group class="mb-3" id="input-group-2" label="Director:" label-for="input-2">
          <b-form-input id="input-2" v-model="pelicula.director" placeholder="Director" />
        </b-form-group>

        <b-form-group class="mb-3" id="input-group-3" label="Fecha de publicación:" label-for="input-3">
          <b-form-input id="input-3" v-model="pelicula.fechaPublicacion" placeholder="Fecha de publicación" type="date" />
        </b-form-group>

        <b-form-group class="mb-3" id="input-group-4" label="Duración:" label-for="input-4">
          <b-input-group>
            <template #append>
              <b-input-group-text>minutos</b-input-group-text>
            </template>

            <b-form-input id="input-4" v-model="pelicula.duracion" placeholder="Duración en minutos" type="number" />
          </b-input-group>
        </b-form-group>

        <b-form-group id="input-group-5" label="Categoria:" label-for="input-5">
          <b-form-select id="input-5" v-model="pelicula.categoria" :options="categorias" />
        </b-form-group>
...
export default {
  ...
  methods: {
    ...
    async onDrop(evt) {
      ...
      } else if (this.pelicula.duracion <= 0 || !this.validField(this.pelicula.duracion)) {
      ...
      try {
        ...
        const selectedCategory = this.categorias.find(categoria => categoria.value === this.pelicula.categoria);
